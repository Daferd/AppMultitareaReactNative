...
client.on('message', function (topic, message) {
  ...
  export default function CustomSwitch ({ indice,name, info, isOn, toggleSwitch, switchStates }) {
    ...
    const onInfoPress = () => {
      ...
      navigation.navigate('ChannelDetail', {indice,name,isOn,switchStates});
    };
    ...
    return (
      <View style={styles.container}>
        ...
        <TouchableOpacity style={styles.iconContainer} onPress={toggleCurrentSwitch}>
          <Ionicons 
            name={isSwitch ? 'power' : 'power-outline'} 
            size={40} 
            color={isSwitch ? '#52f242' : 'gray'} 
          />
        </TouchableOpacity>
      </View>
    );
